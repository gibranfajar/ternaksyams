...
@push('scripts')
    <script>
        ...
        // Tangkap gambar yang dimasukkan ke Trix, simpan dulu
        document.addEventListener("trix-attachment-add", function(event) {
            const attachment = event.attachment;
            if (attachment.file) {
                pendingAttachments.push(attachment); // simpan dulu
            }
        });
        ...
        document.querySelector("form").addEventListener("submit", async function(e) {
            if (pendingAttachments.length > 0) {
                ...
                // Optional: disable tombol & kasih loading state
                ...
                // Kosongkan list biar gak keupload ulang
                pendingAttachments = [];

                // Submit ulang setelah semua gambar keupload
                e.target.submit();
            }
        });
        ...
        async function uploadAttachment(attachment) {
            ...
            try {
                const response = await fetch("{{ route('trix.upload') }}", {
                    method: "POST",
                    headers: {
